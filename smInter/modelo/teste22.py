...
class trieNo(object):
    ...
    def motifBySize(self, listaStr, listaFim, motSize, txConser):
        ...
        for grupo in listaStr:
            ...
            listMersTemp = set(listaTempQuant)
            listMersTemp = list(listaTempQuant)

            listCountMers = [0]*len(listMersTemp)

            for p in range(len(listMersTemp)):
                for q in range(len(listaTempQuant)):
                    if listaTempQuant[p] == listaTempQuant[q]:
                        listCountMers[p] = listCountMers[p] +1



            for r in range(len(listMersTemp)):
                if listCountMers[r] >= minConser:
                    listTempMotif.append(listMersTemp[r])

                    #listaMotivosFim.append(listQtMers[r])
                ...
                listMotGen = list(dict.fromkeys(listTempMotif))
            print("Motifs", listaMotivosFim)
            listaMotivosFim.append(listMotGen.copy())
